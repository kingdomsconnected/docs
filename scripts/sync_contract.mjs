...
export const syncContract = async () => {
  ...
  if (manifest.schemaVersion !== 1 || manifest.slug !== slug || manifest.channel !== channel || !/^[a-f0-9]{64}$/.test(manifest.revision ?? "") || !Array.isArray(manifest.files)) throw new Error("Services returned an invalid scripting contract manifest");
