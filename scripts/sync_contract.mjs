...
export const syncContract = async () => {
  ...
  if (manifest.schemaVersion !== 1 || manifest.slug !== slug || manifest.channel !== channel || !/^[a-f0-9]{64}$/.test(manifest.revision ?? "") || !/^[A-Za-z0-9][A-Za-z0-9._+-]{0,63}$/.test(manifest.version ?? "") || !Array.isArray(manifest.files)) throw new Error("Services returned an invalid scripting contract manifest");
  ...
  try {
    for (const file of manifest.files) {
      const fileUrl = `${apiRoot}/documentation-contracts/${slug}/releases/${manifest.revision}/files/${file.path.split("/").map(encodeURIComponent).join("/")}`;
